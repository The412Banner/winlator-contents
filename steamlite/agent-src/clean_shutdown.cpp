...
void (*g_exit_hook)(const char* reason, int code) = nullptr;
...
BOOL WINAPI ctrl_handler(DWORD type) {
    switch (type) {
        ...
        case CTRL_BREAK_EVENT:
            ...
            if (g_exit_hook) g_exit_hook("console-ctrl", 0);
            return TRUE;
        default:
            return FALSE;
    }
...
void watch_loop() {
    while (g_watch_run.load()) {
        if (::GetFileAttributesA(kSentinelPath) != INVALID_FILE_ATTRIBUTES) {
            ...
            if (g_exit_hook) g_exit_hook("sentinel", 0);
            ::ExitProcess(0);
            return;
        }
        ::Sleep(150);
...
extern "C" void wn_launcher_set_exit_hook(void (*hook)(const char* reason, int code)) {
    g_exit_hook = hook;
}
